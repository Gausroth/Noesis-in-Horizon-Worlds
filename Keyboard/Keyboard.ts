 ...
 */
const KeyboardEvent = new NetworkEvent<{greeting: string}>("KeyboardEvent");
...
class Keyboard extends Component<typeof Keyboard> {
  ...
  private startServer() {
    ...
    this.connectCodeBlockEvent(this.entity, CodeBlockEvents.OnPlayerEnterWorld, (player: Player) => {
      ...
      this.sendNetworkEvent(player, KeyboardEvent, {greeting: `Welcome ${player.name.get()}`});
    });
  }
  ...
  private startClient() {
    ...
    this.connectNetworkEvent(this.world.getLocalPlayer(), KeyboardEvent, data => {
